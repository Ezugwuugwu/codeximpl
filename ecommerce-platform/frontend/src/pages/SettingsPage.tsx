...
function BackIcon({ className = "h-4 w-4" }: SettingsIconProps) {
  return (
    <svg aria-hidden="true" className={className} fill="none" viewBox="0 0 24 24">
...
function SettingsPage() {
  ...
  const openSection = (section: Exclude<SettingsSection, "menu">) => {
  ...
  useEffect(() => {
    ...
    focusField(sectionFocusTargets[activeSection], activeSection === "addresses" ? 180 : 120);
  }, [activeSection]);
  ...
  const activeSectionMeta =
    activeSection === "menu"
      ? null
      : settingsNavigation.find((item) => item.key === activeSection) ?? null;
      ...
      <section className="space-y-4">
        <div className="rounded-3xl border border-slate-200 bg-gradient-to-br from-white to-[#eef7fb] p-5 shadow-sm">
          ...
          <p className="mt-3 text-sm leading-6 text-slate-600">
            {activeSection === "menu"
              ? "Choose one section below. Only that form will open."
              : `You are updating ${activeSectionMeta?.label.toLowerCase()}.`}
          </p>
