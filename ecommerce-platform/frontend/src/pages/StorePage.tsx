...
const pickRandomProducts = (source: Product[], count: number, seed: number) => {
...
function StorePage() {
  ...
  useEffect(() => {
    const rotationTimer = window.setInterval(() => {
      setPanelRotationTick((current) => current + 1);
    }, 5000);
  ...
  const heroShowcase = useMemo<HeroTile[]>(() => {
    const featuredTiles = pickRandomProducts(sortedProducts, 4, panelRotationTick + 11).map((product) => ({
      title: product.name,
  ...
  }, [sortedProducts, panelRotationTick]);

  const filteredProducts = useMemo(() => {
    const query = allProductsQuery.trim();
    const source = query ? (searchResults ?? []) : sortedProducts;
    ...
    return source.filter((product) => {
      ...
      const categoryMatches = normalizedSelectedCategory
        ...
        : true;
      return categoryMatches && matchesSearch(product, query);
    });
  }, [sortedProducts, searchResults, allProductsQuery, normalizedSelectedCategory]);
      ...
      {loading ? (
        ...
        <>
          <section id="products" className="space-y-4">
            ...
            {hasMore && !allProductsQuery && !selectedCategory && (
              <div className="flex justify-center pt-2">
                <button
                  className="rounded-full border border-slate-300 bg-white px-6 py-2 text-sm font-medium text-slate-700 shadow-sm hover:bg-slate-50 disabled:opacity-50"
